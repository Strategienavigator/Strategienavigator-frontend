...
import {PersonaFactors2} from "./PersonaFactors";
...
export interface PersonaFactorsValues2 {
    factors: {
...
export class PersonaFactorsComponent extends Step<PersonaAnalysisValues, PersonaFactorsState> {
    ...
    shouldComponentUpdate(nextProps: Readonly<StepProp<PersonaAnalysisValues>>, nextState: Readonly<PersonaFactorsState>, nextContext: IUIErrorContext): boolean {
        ...
        if (!shouldUpdate) {
            ...
            if (oldSave.data["persona-factors2"] !== newSave.data["persona-factors2"]) {
                shouldUpdate = true;
            }
        }
        return shouldUpdate;
    ...
    build(): JSX.Element {
        const min = PersonaFactors2.min;
        const max = PersonaFactors2.max;
        let activeKey = "view";

        let values = this.props.save.data["persona-factors"]?.factors;
        if (values !== undefined) {
            return (
                <div className={"persona-factors2"}>
                    <Accordion flush={true} activeKey={this.props.validationFailed ? activeKey : undefined}
                        ...
                        <Accordion.Item eventKey={this.props.validationFailed ? activeKey : "qualifikation"}>
                            <Accordion.Header>{PersonaFactors2.qualifikationCounter.get(1) + "-" + PersonaFactors2.qualifikationCounter.get(max)} -
                            Qualifikation</Accordion.Header>
                            <Accordion.Body>
                                <CardComponent required={false}
                                               values={values.qualifikation}
                                               counter={PersonaFactors2.qualifikationCounter}
                                               name={"qualifikation"}
                        ...
                        <Accordion.Item eventKey={this.props.validationFailed ? activeKey : "art_der_Erkrankung"}>
                            <Accordion.Header>{PersonaFactors2.art_der_ErkrankungCounter.get(1) + "-" + PersonaFactors2.art_der_ErkrankungCounter.get(max)} -
                            Art_der_Erkrankung</Accordion.Header>
                            <Accordion.Body>
                                <CardComponent required={false}
                                               values={values.art_der_Erkrankung}
                                               counter={PersonaFactors2.art_der_ErkrankungCounter}
                        ...
                        <Accordion.Item eventKey={this.props.validationFailed ? activeKey : "beraterOrAngehörige"}>
                            <Accordion.Header>{PersonaFactors2.beraterOrAngehörigeCounter.get(1) + "-" + PersonaFactors2.beraterOrAngehörigeCounter.get(max)} -
                            Berater oder Angehörige</Accordion.Header>
                            <Accordion.Body>
                                <CardComponent required={false}
                                               values={values.beraterOrAngehörige}
                                               counter={PersonaFactors2.beraterOrAngehörigeCounter}
                        ...
                        <Accordion.Item eventKey={this.props.validationFailed ? activeKey : "familieOrFreunde"}>
                            <Accordion.Header>{PersonaFactors2.familieOrFreundeCounter.get(min) + "-" + PersonaFactors2.familieOrFreundeCounter.get(max)} -
                            Familie oder Freunde</Accordion.Header>
                            <Accordion.Body>
                                <CardComponent required={false}
                                               values={values.familieOrFreunde}
                                               counter={PersonaFactors2.familieOrFreundeCounter}
                        ...
                        <Accordion.Item eventKey={this.props.validationFailed ? activeKey : "charaktereigenschaften"}>
                            <Accordion.Header>{PersonaFactors2.charaktereigenschaftenCounter.get(1) + "-" + PersonaFactors2.charaktereigenschaftenCounter.get(max)} -
                            Charaktereigenschaften</Accordion.Header>
                            <Accordion.Body>
                                <CardComponent required={false}
                                               values={values.charaktereigenschaften}
                                               counter={PersonaFactors2.charaktereigenschaftenCounter}
                        ...
                        <Accordion.Item eventKey={this.props.validationFailed ? activeKey : "bedürfnisse"}>
                            <Accordion.Header>{PersonaFactors2.bedürfnisseCounter.get(1) + "-" + PersonaFactors2.bedürfnisseCounter.get(max)} -
                            Bedürfnisse</Accordion.Header>
                            <Accordion.Body>
                                <CardComponent required={false}
                                               values={values.bedürfnisse}
                                               counter={PersonaFactors2.bedürfnisseCounter}
                                               name={"bedürfnisse"}
                                               disabled={this.props.disabled}
                                               min={min}
                                               max={max}
                       ...
                       <Accordion.Item eventKey={this.props.validationFailed ? activeKey : "hobies"}>
                            <Accordion.Header>{PersonaFactors2.hobiesCounter.get(1) + "-" + PersonaFactors2.hobiesCounter.get(max)} -
                            Hobies</Accordion.Header>
                            <Accordion.Body>
                                <CardComponent required={false}
                                               values={values.hobies}
                                               counter={PersonaFactors2.hobiesCounter}
                                               name={"hobies"}
                                               disabled={this.props.disabled}
                                               min={min}
                                               max={max}
                         ...
                         <Accordion.Item eventKey={this.props.validationFailed ? activeKey : "motivation"}>
                            <Accordion.Header>{PersonaFactors2.motivationCounter.get(1) + "-" + PersonaFactors2.motivationCounter.get(max)} -
                            Motivation</Accordion.Header>
                            <Accordion.Body>
                                <CardComponent required={false}
                                               values={values.motivation}
                                               counter={PersonaFactors2.motivationCounter}
                                               name={"motivation"}
                                               disabled={this.props.disabled}
                                               min={min}
                                               max={max}
                        ...
                        <Accordion.Item eventKey={this.props.validationFailed ? activeKey :"zitat"}>
                            <Accordion.Header>{PersonaFactors2.zitatCounter.get(1) + "-" + PersonaFactors2.zitatCounter.get(max)} -
                            Zitat</Accordion.Header>
                            <Accordion.Body>
                                <CardComponent required={false}
                                               values={values.zitat}
                                               counter={PersonaFactors2.zitatCounter}
                                               name={"zitat"}
                                               disabled={this.props.disabled}
                                               min={min}
                                               max={max}
    ...
    private applyCardComponentChanges(type: String , values: any) {
        this.props.saveController.onChanged(save => {
            const data = save.data["persona-factors2"];
            if (data !== undefined) {
                switch (type) {
                    case "qualifikation":
